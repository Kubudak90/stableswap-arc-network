...
async function main() {
  ...
  // Transfer amounts (100,000 tokens each - BIG AMOUNT!)
  const transferAmount = hre.ethers.parseUnits("100000", 6); // 6 decimals
  ...
  console.log("\n=== Transferring BIG AMOUNT ===");
  
  // Transfer tUSDC
  console.log("Transferring 100,000 tUSDC...");
  const tx1 = await usdcContract.transfer(recipient, transferAmount);
  await tx1.wait();
  console.log("✅ tUSDC transfer completed");

  // Transfer tUSDT
  console.log("Transferring 100,000 tUSDT...");
  const tx2 = await usdtContract.transfer(recipient, transferAmount);
  await tx2.wait();
  console.log("✅ tUSDT transfer completed");

  console.log("\n=== Final Balances ===");
  const finalRecipientUSDC = await usdcContract.balanceOf(recipient);
  const finalRecipientUSDT = await usdtContract.balanceOf(recipient);
  console.log("Recipient tUSDC:", hre.ethers.formatUnits(finalRecipientUSDC, 6));
  console.log("Recipient tUSDT:", hre.ethers.formatUnits(finalRecipientUSDT, 6));

  console.log("\n🎉 BIG AMOUNT transferred successfully!");
}
